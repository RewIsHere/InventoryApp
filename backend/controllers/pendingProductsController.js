...
export const listPendingProducts = async (req, res) => {
  try {
    ...
    // Filtrar por coincidencias en barcode o movement_id
    if (search) {
      query = query.or(`barcode.ilike.%${search}%`);
    }
...
export const getPendingProductDetails = async (req, res) => {
  try {
    ...
    if (!data)
      return res
        .status(404)
        .json({ error: "Producto pendiente no encontrado." });
  ...
  try {
    ...
    if (!name || typeof name !== "string") {
      return res.status(400).json({
        error: "El campo 'name' es obligatorio y debe ser una cadena de texto.",
      });
    }
    if (!min_stock || typeof min_stock !== "number" || min_stock < 0) {
      return res.status(400).json({
        error:
          "El campo 'min_stock' es obligatorio y debe ser un número mayor o igual a cero.",
      });
    }
    if (!category_id || typeof category_id !== "string") {
      return res.status(400).json({
        error:
          "El campo 'category_id' es obligatorio y debe ser un UUID válido.",
      });
    }
    ...
    if (!pending) {
      return res.status(400).json({
        error: "No se encontro ningun producto pendiente con este ID.",
      });
    }
    ...
    if (remainingUnregistered.length === 0) {
      ...
      if (movementFetchError)
        return res.status(500).json({ error: movementFetchError.message });
      if (!movementData) {
        return res.status(404).json({
          error:
            "Movimiento no encontrado o no está en estado COMPLETED_WITH_UNREGISTERED.",
        });
      }
