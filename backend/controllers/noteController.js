...
export const createNote = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Nota creada correctamente.", note: data[0] });
  } catch (err) {
...
export const listNotes = async (req, res) => {
  try {
    ...
    // Obtener las notas del producto con los detalles del usuario
    const { data, error } = await supabase
      .from("product_notes")
      .select("id, note, created_at, users(name, surnames)") // Incluir name y surnames del usuario
      .eq("product_id", id)
    ...
    // Si no hay notas para el producto, devolver un array vacío
    if (!data || data.length === 0) {
      return res.status(200).json([]);
    }

    // Transformar los datos para incluir el nombre completo del usuario
...
export const updateNote = async (req, res) => {
  try {
    ...
    if (existingNote.product_id !== productId) {
      return res
        .status(400)
        .json({ error: "La nota no pertenece a este producto." });
    }
