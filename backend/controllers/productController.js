...
export const createProduct = async (req, res) => {
    try {
        const { name, description, barcode, stock, min_stock, status, category_id } = req.body; // Agregar category_id
        const userId = req.user.id;

        // Validar que category_id esté presente
        if (!category_id) {
            return res.status(400).json({ error: "El campo category_id es obligatorio." });
        }
        ...
        const { error: historyError } = await supabase.from('product_history').insert({
            ...
            details: { name, description, barcode, stock, min_stock, status, category_id } // Incluir category_id en el historial
        });
        ...
        res.status(201).json({ message: "Producto creado correctamente.", product: data[0] });
    } catch (err) {
...
export const updateProduct = async (req, res) => {
    try {
        ...
        if (category_id && category_id !== oldProduct.category_id) {
            ...
            historyDetails.category_id = { old: oldProduct.category_id, new: category_id };
        }
...
export const uploadImage = async (req, res) => {
    try {
        ...
        if (!allowedMimeTypes.includes(file.mimetype)) {
            return res.status(400).json({ error: 'Tipo de archivo no soportado. Solo se permiten imágenes JPEG, PNG y JPG.' });
        }
