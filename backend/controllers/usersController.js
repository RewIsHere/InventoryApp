...
export const deleteUser = async (req, res) => {
  try {
    ...
    if (error.message === "Cannot delete the superadmin account") {
      return res
        .status(403)
        .json({ error: "Cannot delete the superadmin account" });
    }
    if (error instanceof ZodError) {
      return res
        .status(400)
...
export const toggleUserActiveStatus = async (req, res) => {
  try {
    ...
    if (error.message === "Cannot deactivate the superadmin account") {
      return res
        .status(403)
        .json({ error: "Cannot deactivate the superadmin account" });
    }
    if (error instanceof ZodError) {
      return res
        .status(400)
...
export const updateUserByAdmin = async (req, res) => {
  try {
    ...
    if (error.message === "Cannot modify the role of a superadmin account") {
      return res
        .status(403)
        .json({ error: "Cannot modify the role of a superadmin account" });
    }
